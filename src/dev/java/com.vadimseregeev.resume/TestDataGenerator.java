package com.vadimseregeev.resume;

import com.vadimsergeev.resume.model.LanguageLevel;
import com.vadimsergeev.resume.model.LanguageType;
...
public class TestDataGenerator {
    ...
    private static final String COUTRY = "Latvia";
    private static final String[] CITIES = {"Riga", "Jelgava", "Valmiera"};
    private static final String[] FOREGIN_LANGUAGES = {"Latvian", "Russian", "English", "Dutch"};
    ...
    private static void insertProfileData(Connection c, Profile profile, ProfileConfig profileConfig) throws SQLException, IOException {
        ...
        String path = (profile.firstName + "-" + profile.lastName).toLowerCase();
        ps.setString(1, path);
        ...
        if (r.nextBoolean()) {
            ps.setString(17, "https://vk.com/" + path);
        } else {
            ps.setNull(17, Types.VARCHAR);
        }
        if (r.nextBoolean()) {
            ps.setString(18, "https://facebook.com/" + path);
        } else {
            ps.setNull(18, Types.VARCHAR);
        }
        if (r.nextBoolean()) {
            ps.setString(19, "https://linkedin.com/" + path);
        } else {
            ps.setNull(19, Types.VARCHAR);
        }
        if (r.nextBoolean()) {
            ps.setString(20, "https://github.com/" + path);
        } else {
            ps.setNull(20, Types.VARCHAR);
        }
        if (r.nextBoolean()) {
            ps.setString(21, "https://stackoverflow.com/" + path);
        } else {
